...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.add-collection-form').forEach(form => {
        form.addEventListener('submit', async (e) => {
            ...
            try {
                const response = await fetch('add_to_collection_ajax.php', {
                    method: 'POST',
                    body: formData
                });
    ...
    document.querySelectorAll('.add-wishlist-form').forEach(form => {
        form.addEventListener('submit', async (e) => {
            ...
            const formData = new FormData(form);
            try {
                const response = await fetch('add_to_wishlist_ajax.php', {
                    method: 'POST',
                    body: formData
                });
                const result = await response.json();
    ...
    document.querySelectorAll('.add-deck-form').forEach(form => {
        form.addEventListener('submit', async (e) => {
            ...
            try {
                const res  = await fetch('add_to_deck_ajax.php', { method: 'POST', body: fd });
                const data = await res.json();
                if (data.success) {
                    showToast(
